...
async function debugPL() {
    try {
        ...
        if (Array.isArray(report)) {
            const flattened = findAccountsRecursive(report);
            console.log('Flattened Accounts:');
            flattened.forEach(acc => {
                console.log(`- ${acc.name} (Code: ${acc.code || 'N/A'}): ${acc.amount}`);
            });
        } else {
            console.log('Report is an object.');
            const flattened = [...(report.income_accounts || []), ...(report.expense_accounts || [])];
            flattened.forEach(acc => {
                console.log(`- ${acc.account_name} (Code: ${acc.account_code || 'N/A'}): ${acc.amount}`);
            });
        }
