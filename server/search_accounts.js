...
const findAccountsRecursive = (transactions) => {
    ...
    transactions.forEach(item => {
        if (item.account_name || (item.name && item.account_id)) {
            accounts.push({
                name: (item.account_name || item.name || '').toLowerCase(),
...
async function debugPL() {
    try {
        ...
        flattened.forEach(acc => {
            ...
            if (name.includes('visa') || name.includes('langue') || name.includes('livre') || name.includes('billet') || name.includes('70')) {
                console.log(`- ${name} (Code: ${code}): ${acc.amount}`);
            }
        });
